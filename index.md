...
\\f(x \mid \alpha, \beta, \gamma) \\\propto\\ x^{\alpha - 1} \exp(-\beta
x^2 + \gamma x), \qquad x \> 0,\\

where \\\alpha, \beta \> 0\\ and \\\gamma \in \mathbb{R}\\. It arises as
a conditional posterior in Bayesian MCMC for several common models
(skew-elliptical regression, \\t\\-shrinkage priors, log-concave
...
| [`mhn_var()`](https://t-momozaki.github.io/mhn/reference/mhn_var.md) | Variance, Sun et al. (2023, Lemma 2c) where that form is well conditioned |
...
special cases are taken first whatever it is set to: a negligible tilt
draws from the square root of a Gamma, and α ≈ 1 from a truncated
normal. Both are exact and cheaper than any rejection scheme.
