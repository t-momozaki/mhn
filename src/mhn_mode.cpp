...
// boundary not directly covered by Lemma 3).
...
// [[Rcpp::export(.mhn_mode_cpp, rng = false)]]
double mhn_mode_cpp(double alpha, double beta, double gamma) {
  ...
  if (alpha > 1.0 + eps) {
    ...
    return mhn::positive_root(beta, gamma, alpha - 1.0);
  }
  ...
  if (gamma > 0.0 && alpha >= 1.0 - gamma * gamma / (8.0 * beta) - eps) {
    // Here alpha < 1, so the "c" of positive_root is negative and the
    // discriminant can vanish; check it before taking the root.
